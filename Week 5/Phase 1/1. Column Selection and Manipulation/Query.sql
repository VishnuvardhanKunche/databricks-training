...
SELECT product_name
FROM products; -- Output 1

-- 2. Selecting multiple columns
SELECT product_id, product_name, price
FROM products; -- Output 2

-- 3. Selecting columns (Equivalent to col() in PySpark)
SELECT product_id, product_name, price
FROM products; -- Output 3

-- 4. Adding a new column with a static value
SELECT *, 0 AS discount
FROM products; -- Output 4

-- 5. Adding a new column based on an existing one
SELECT *, price * 0.10 AS discount
FROM products; -- Output 5

-- 6. Adding multiple new columns
SELECT *, price * 0.50 AS discount, price - (price * 0.50) AS new_price
FROM products; -- Output 6

-- 7. Renaming columns using aliases
SELECT product_id AS id,  product_name AS product
FROM products; -- Output 7

-- 8. Renaming a single column
SELECT product_id AS id,
       product_name,
       category,
       price,
       stock_quantity
FROM products; -- Output 8
...
SELECT product_id AS id,
       ...
       price,
       stock_quantity
FROM products; -- Output 9

-- 10. Selecting with expressions
SELECT product_name,
       stock_quantity + 10 AS new_stock_quantity
FROM products; -- Output 10

-- 11. Selecting all columns except price
SELECT product_id,
       product_name,
       category,
       stock_quantity
FROM products; -- Output 11

-- 12. Using SQL syntax to select columns
SELECT product_name,
       category
FROM products; -- Output 12

-- 13. Dynamic column selection (Equivalent)
SELECT product_name,
       category
FROM products; -- Output 12

-- 14. Excluding price and stock_quantity
SELECT product_id,
       product_name,
       category
FROM products; -- Output 13